...
This file defines guidelines for programming in C++ for ESP-IDF projects using Kiro IDE. These practices ensure that the code is robust, maintainable, and efficient, optimized for the ESP32 microcontroller.
...
- **Naming Conventions**:
  ...
  - Prefer descriptive names, aligning with ESP-IDF conventions (e.g., `initGpio` instead of `initializeGpio`).
- **Comments**:
  - Include comments to explain the logic of each function, class, or significant code block.
...
- **Error Codes**:
  ...
  - Check return values of critical functions and handle errors appropriately.
  - Example:
    ```cpp
